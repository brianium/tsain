...
- `::tsain/replace-css` optional args - can now call without options map
- `::tsain/split-css` optional args - can now call without options map
- `::tsain/write-css` optional args - can now call without options map

## [0.6.1] - 2026-01-31

### Fixed

- `find-rule-end-line` infinite loop caused by nested loop `recur` targeting wrong scope
- `find-selector-line` regex anchor preventing matches inside `@media` blocks
- `::tsain/replace-css` now correctly removes matched rules before appending new content
